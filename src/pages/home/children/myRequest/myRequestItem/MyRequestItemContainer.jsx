...
import { StyleMyRequestItem } from './StyleMyRequestItem'
import RequestCardUI from '../requestCard/RequestCardContainer'

class MyRequestItemUI extends Component {
  constructor(props) {
    super(props);
    this.state = {
      leave: [],
  ...
  async componentDidMount() {
    ...
    for (let i = 0; i < this.state.reim.length; i++) {
      ...
      let btime = betime.toLocaleString()
      let etime = entime.toLocaleString()
      let xtime = (this.state.reim[i].reimFinaltime - this.state.reim[i].reimCreatetime)
      let x = Math.floor(xtime / (1000 * 60 * 60 * 24) + 1)
    ...
    for (let i = 0; i < this.state.trip.length; i++) {
      ...
      let btime = betime.toLocaleString()
      let etime = entime.toLocaleString()
      let xtime = (this.state.trip[i].worEnddate - this.state.trip[i].worBegindate)
      let x = Math.floor(xtime / (1000 * 60 * 60 * 24) + 1)
  ...
  render() {
    ...
    for (let i = 0; i < this.state.total.length; i++) {
      tempCard[i] = <RequestCardUI {...this.state.total[i]}></RequestCardUI>;
    }
    return (
      <StyleMyRequestItem>
...
export default MyRequestItemUI;